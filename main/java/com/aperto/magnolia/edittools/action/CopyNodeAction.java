...
public class CopyNodeAction extends AbstractRepositoryAction<CopyNodeActionDefinition> {
    ...
    protected String getComponentName(final JcrItemAdapter node) {
        ...
        if (templateId != null) {
            DefinitionProvider<TemplateDefinition> definition = _templateRegistry.getProvider(String.valueOf(templateId.getValue()));
            name = _translationService.translate(_localeProvider, new String[]{definition.get().getTitle()});
        }
